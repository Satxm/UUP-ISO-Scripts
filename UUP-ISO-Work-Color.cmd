...
if %_build% geq 22000 if %_build% lss 26052 if %LCUWinre% equ 2 (set LCUWinre=0) else (set LCUWinre=1)
...
if exist "!_DIR!\Apps\*_8wekyb3d8bbwe" if %_Srvr% neq 1 if not exist "!_DIR!\Apps\Custom_Appxs.txt" if not exist "!_DIR!\Apps\Apps_*.txt" call :appx_sort
...
echo.
echo [Channel]
if %_LTSC% equ 1 (echo Volume) else (echo Retail)
echo.
echo [VL]
...
if %SkipISO% neq 0 (
  ...
  call :dk_color1 %Green% "已完成要求的操作。你已选择不创建 .iso 文件。" 4
  goto :QUIT
)
...
if %AddUpdates% neq 1 if %AddAppxs% neq 1 if not defined DrvSrcAll if not defined DrvSrcOS if %_Srvr% equ 1 goto :SkipUpdate
...
:DoESD
call :dk_color1 %Blue% "=== 正在将 install.wim 转换为 install.esd..." 4 5
...
:DoSWM
call :dk_color1 %Blue% "=== 正在将 install.wim 拆分为多个 install*.swm..." 4 5
wimlib-imagex.exe split ISOFOLDER\sources\install.wim ISOFOLDER\sources\install.swm 3500
call set ERRTEMP=!ERRORLEVEL!
if !ERRTEMP! neq 0 (
  call :dk_color1 %Red% "在拆分映像的时候出现错误。正在丢弃 install.swm" 4
...
set "_inx=1"&call :DoMount "temp\Winre.wim"
call :Cleanup
call :DoWork
call :DoUnmount
goto :WinreDone
...
:upd_msu
call :dk_color1 %Blue% "创建累积更新的 MSU 文件" 4 5
pushd "!_DIR!"
...
if exist "*Windows1*%_MSUkbn%*%arch%*.msu" (
  call :dk_color1 %_Yellow% "累积更新 %_MSUkbn% 的 msu 文件已经存在，将跳过操作。"
  goto :eof
)
...
if %ERRORLEVEL% neq 0 (
  call :dk_color1 %_Yellow% "makecab.exe 对 %_MSUonf% 操作失败，将跳过操作。" 4
  goto :eof
)
...
if %ERRORLEVEL% neq 0 (
  call :dk_color1 %_Yellow% "makecab.exe 对 %_MSUkbf%.msu 操作失败，将跳过操作。" 4
  goto :eof
)
goto :eof
...
if %ERRORLEVEL% neq 0 (
  call :dk_color1 %_Yellow% "捕获 %_MSUkbf%.msu 操作失败，将跳过操作。" 4
  goto :eof
)
goto :eof
...
%_Nul3% makecab.exe /F zzz.ddf /D Compress=ON /D CompressionType=MSZIP
if %ERRORLEVEL% neq 0 (
  call :dk_color1 %_Yellow% "由于 makecab.exe 对 %_MSUddc% 操作失败，将跳过操作。" 4
  set _mcfail=1
  exit /b
)
...
%_Nul3% makecab.exe /F zzz.ddf /D Compress=ON /D CompressionType=MSZIP
if %ERRORLEVEL% neq 0 (
  call :dk_color1 %_Yellow% "由于 makecab.exe 对 %_MSUddd% 操作失败，将跳过操作。" 4
  set _mcfail=1
  exit /b
)
rmdir /s /q "_tSSU\" %_Nul3%
...
set _cab=0
if %_build% geq 21382 if exist "!_DIR!\*Windows1*-KB*.msu" for /f "tokens=* delims=" %%# in ('dir /b /on "!_DIR!\*Windows1*-KB*.msu"') do (set "package=%%#"&call :sum2msu)
...
if exist "!_DIR!\RCU-*-*.cab" for /f "tokens=* delims=" %%# in ('dir /b /on "!_DIR!\RCU-*-*.cab"') do (call set /a _cab+=1)
...
if %_build% geq 21382 if exist "!_DIR!\*Windows1*-KB*.msu" for /f "tokens=* delims=" %%# in ('dir /b /on "!_DIR!\*Windows1*-KB*.msu"') do (set "pkgn=%%~n#"&set "package=%%#"&set "dest=!_cabdir!\%%~n#"&call :msu2)
...
if exist "!_cabdir!\*Windows1*-KB*.wim" for /f "tokens=* delims=" %%# in ('dir /b /on "!_cabdir!\*Windows1*-KB*.wim"') do (set "pkgn=%%~n#"&set "package=%%#"&set "dest=!_cabdir!\%%~n#"&call :psfx2)
if exist "!_DIR!\RCU-*-*.cab" for /f "tokens=* delims=" %%# in ('dir /b /on "!_DIR!\RCU-*-*.cab"') do (set "pkgn=%%~n#"&set "package=%%#"&set "dest=!_cabdir!\%%~n#"&call :cab2)
goto :eof

:sum2msu
expand.exe -d -f:*Windows*.psf "!_DIR!\%package%" %_Nul2% | findstr /i %arch%\.psf %_Nul3% || (
  wimlib-imagex.exe dir "!_DIR!\%package%" %_Nul2% | findstr /i %arch%\.psf %_Nul3% || goto :eof
)
call set /a _cab+=1
goto :eof
...
set "uupmsu="
if %_build% geq 21382 if exist "!_DIR!\*Windows1*%pkgid%*%arch%*.msu" for /f "tokens=* delims=" %%# in ('dir /b /on "!_DIR!\*Windows1*%pkgid%*%arch%*.msu"') do set "uupmsu=%%#"
if defined uupmsu (
...
if exist "!dest!\*.psf.cix.xml" (
  if not exist "!_DIR!\%pkgn%.psf" if not exist "!_DIR!\*%pkgid%*%arch%*.psf" (
    call :dk_color1 %Red% "[%count%/%_cab%] %package% / PSF 文件丢失"
    goto :eof
  )
  if %psfnet% equ 0 (
    call :dk_color1 %Red% "[%count%/%_cab%] %package% / PSFExtractor 不可用"
    goto :eof
  )
  set psf_%pkgn%=1
)
...
if exist "!dest!\toc.xml" (
  ...
  if exist "!_cabdir!\lcu\*Windows1*-KB*.cab" for /f "tokens=* delims=" %%# in ('dir /b /on "!_cabdir!\lcu\*Windows1*-KB*.cab"') do (set "compkg=%%#"&call :inrencab)
...
if not defined _type (
  findstr /i /m "Package_for_RevisedFix" "!dest!\update.mum" %_Nul3% && (set "_type=[修订累积更新]"&set uwinpe=1)
)
if not defined _type (
...
if defined wim_%pkgn% (
  ...
  if !errorlevel! neq 0 (
    call :dk_color1 %Red% "出现错误：解压 WIM 更新包失败"
    rmdir /s /q "!dest!\" %_Nul3%
    set wim_%pkgn%=
    goto :eof
  )
...
if exist "!_cabdir!\lcu\HotpatchCompDB*.cab" (
  call :dk_color1 %_Yellow% "不支持的更新：%package% [热修复更新]" 4
  rmdir /s /q "!_cabdir!\lcu\" %_Nul3%
  goto :eof
)
set /a count+=1
...
:psfx2
if defined psf_%pkgn% goto :eof
for /f "tokens=2 delims=-" %%V in ('echo %pkgn%') do set pkgid=%%V
if exist "!dest!\" rmdir /s /q "!dest!\"
mkdir "!dest!"
set "pkgm=nein"
if exist "!_DIR!\*%pkgid%*.msu" (
for /f "tokens=* delims=" %%# in ('dir /b /on "!_DIR!\*%pkgid%*.msu"') do set "pkgm=%%~n#"
) else if defined _rcu (
  if exist "!_DIR!\*%_rcu%*.msu" for /f "tokens=* delims=" %%# in ('dir /b /on "!_DIR!\*%_rcu%*.msu"') do set "pkgm=%%~n#"
)
set msu_%pkgm%=
set psf_%pkgn%=1
set /a count+=1
...
:inrencab
call set /a _cab+=1
if exist "!_DIR!\%compkg%" del /f /q "!_DIR!\%compkg%" %_Nul3%
...
if %winbuild% lss 15063 if not exist "%_mount%\Windows\Servicing\Packages\*WinPE-LanguagePack*.mum" (
  call :RegLoad
...
if defined mpamfe (
  call :dk_color1 %Blue% "=== 正在添加 Defender 更新..." 4 5
...
:svcpkg
set "kbnm=%~1"
%_Nul3% icacls "%_mount%\Windows\Servicing\Packages\%kbnm%" /save "!_cabdir!\acl.txt"
%_Nul3% takeown /f "%_mount%\Windows\Servicing\Packages\%kbnm%" /A
%_Nul3% icacls "%_mount%\Windows\Servicing\Packages\%kbnm%" /grant *S-1-5-32-544:F
%_Nul3% copy /y "!_cabdir!\LCUmum\%kbnm%" "%_mount%\Windows\Servicing\Packages\%kbnm%"
%_Nul3% icacls "%_mount%\Windows\Servicing\Packages\%kbnm%" /setowner *S-1-5-80-956008885-3418522649-1831038044-1853292631-2271478464
...
call :dk_color1 %Red% "%msgerr%。正在丢弃当前挂载镜像..."
...
findstr /i HKEY_LOCAL_MACHINE "temp\Reg-Similar.txt" %_Nul3% && for %%# in (%basekbn%) do (
  for /f "usebackq tokens=8 delims=\" %%G in ("temp\Reg-Similar.txt") do (
    reg.exe query "%_k_%\%%G" /v InstallLocation %_Nul2% | findstr /i %%# %_Nul1% && echo %%G >>"temp\Reg-Matched.txt"
  )
)
...
if /i "%packx%"==".wim" set xwim=1
for /f "tokens=2 delims=-" %%V in ('echo %pckn%') do set pckid=%%V
set "uupmsu="
if %xmsu% equ 0 if %xwim% equ 0 if %_build% geq 21382 if exist "!_DIR!\*Windows1*%pckid%*%arch%*.msu" for /f "tokens=* delims=" %%# in ('dir /b /on "!_DIR!\*Windows1*%pckid%*%arch%*.msu"') do set "uupmsu=%%#"
if defined uupmsu (
  expand.exe -d -f:*Windows*.psf "!_DIR!\%uupmsu%" %_Nul2% | findstr /i %arch%\.psf %_Nul3% && goto :eof
  wimlib-imagex.exe dir "!_DIR!\%uupmsu%" %_Nul2% | findstr /i %arch%\.psf %_Nul3% && goto :eof
)
...
for %%# in (%directcab%) do (
  if /i "%package%"=="%%~#" (
    set "cumulative=!cumulative! "!_DIR!\%package%""
    goto :eof
  )
)
if exist "!dest!\update.mum" findstr /i /m "Package_for_RollupFix" "!dest!\update.mum" %_Nul3% && goto :proclcu
if exist "!dest!\update.mum" findstr /i /m "Package_for_RevisedFix" "!dest!\update.mum" %_Nul3% && goto :procrcu
if exist "%_mount%\Windows\Servicing\Packages\*WinPE-LanguagePack*.mum" (
...
reg add "HKLM\%USER%\Software\Microsoft\Windows\CurrentVersion\Start" /v "AllAppsViewMode" /t REG_DWORD /d 1 /f %_Nul3%
reg add "HKLM\%USER%\Software\Microsoft\Windows\CurrentVersion\Start" /v "ShowAllPinsList" /t REG_DWORD /d 1 /f %_Nul3%
reg add "HKLM\%USER%\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced" /v "ShowSecondsInSystemClock" /t REG_DWORD /d 1 /f %_Nul3%
reg unload HKLM\%USER% %_Nul3%
goto :eof
...
:appx_sort
call :dk_color1 %Blue% "=== 正在解析应用 CompDB 信息..." 4 5
if %_pwsh% equ 0 (
call :dk_color1 %_Yellow% "未检测到 Windows PowerShell，将跳过操作。"
goto :eof
)
pushd "!_DIR!"
...
call :dk_color1 %_Yellow% "由于 CompDB_App.xml 文件没有找到，将跳过操作。"
...
set "_stub="
if %_Srvr% neq 1 if %AppsAsStub% equ 1 if exist "%_pfn%\AppxMetadata\Stub\*.*x" set "_stub=/StubPackageOption:InstallStub"
set Dependency=
...
:RemoveAppx
call :dk_color1 %Blue% "=== 正在卸载 Appxs 软件包..." 4 5

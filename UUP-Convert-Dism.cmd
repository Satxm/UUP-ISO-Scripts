...
:checkdone
echo.
if defined _args for %%# in (%*) do if exist "%%~#\*.esd" (set "_DIR=%%~f#"&echo %%~#&goto :checkesd)
for /f "tokens=* delims=" %%# in ('dir /b /ad "!_work!"') do if exist "%%~#\*.esd" (set /a _ndir+=1&set "_DIR=%%~f#"&echo %%~#)
...
for /L %%# in (1, 1,%_nsum%) do (
  ...
  %_Nul3% %_psc% "$Filenew = Get-Item '!_DIR!\!uups_esd%%#!';$Fileold = Get-Item '!_DIR!\CanonicalUUP\!uups_esd%%#!';$Filenew.LastWriteTime = $Fileold.LastWriteTime"
)
exit /b
